...
try {
  ...
  ok('clamp A: Show more removes the clamp; entry at full height; control now reads Show less, aria-expanded=true',
     k.e[3] && !k.e[3].clamped && k.e[3].more && k.e[3].more.text === 'Show less' && k.e[3].more.expanded === 'true' && k.e[3].client === k.e[3].scroll, JSON.stringify(k.e[3]));
  let fm = await J(`JSON.stringify({cls:document.activeElement.className, idx:document.activeElement.closest('.gentry')?.dataset.idx})`);
  ok('focus: after Show more, focus stays on the toggle', /gmore/.test(fm.cls) && fm.idx === '3', JSON.stringify(fm));
  await evalJs(`renderAll(); true`);
  await sleep(150);
  k = await J(clampProbe(A));
  ok('clamp A: opened entry survives a full renderAll() (live-reload re-render), Show less still offered',
     k.e[3] && !k.e[3].clamped && k.e[3].more && k.e[3].more.text === 'Show less', JSON.stringify(k.e[3]));
  // Show less: re-clamps in place, survives re-render as clamped.
  await evalJs(`document.querySelector(${JSON.stringify(A)} + ' .gentry[data-idx="3"] .gmore').click(); renderAll(); true`);
  await sleep(150);
  k = await J(clampProbe(A));
  ok('clamp A: Show less re-clamps the entry, control reads Show more again, and that survives renderAll()',
     k.e[3] && k.e[3].clamped && k.e[3].more && k.e[3].more.text === 'Show more' && k.e[3].more.expanded === 'false' && Math.abs(k.e[3].client - 8 * k.e[3].lh) < 1, JSON.stringify(k.e[3]));
  await evalJs(`document.querySelector(${JSON.stringify(A)} + ' .gentry[data-idx="3"] .gmore').click(); true`);
  await sleep(100);
  ...
  await sleep(150);
  const f0 = await J(`JSON.stringify({cls:document.activeElement.className, inA:!!document.activeElement.closest(${JSON.stringify(A)})})`);
  ok('focus: after unfolding, focus stays on the fold toggle', f0.inA && /gfold/.test(f0.cls), JSON.stringify(f0));
  ...
  ok('case A: the toggle stays, reading "Hide 2 earlier replies", aria-expanded=true', a.present && a.fold && a.fold.text.trim() === 'Hide 2 earlier replies' && a.fold.expanded === 'true', JSON.stringify(a));
  ...
  await sleep(150);
  a = await J(cardProbe(A));
  ok('case A: expanded state survives a full renderAll() (live-reload re-render)', a.present && a.n === 5 && a.fold && a.fold.expanded === 'true', JSON.stringify(a));
  // Re-fold: the toggle folds the thread again, and that survives a re-render too.
  await evalJs(`document.querySelector(${JSON.stringify(A)} + ' .gfold').click(); renderAll(); true`);
  await sleep(150);
  a = await J(cardProbe(A));
  ok('case A: clicking the toggle again re-folds (root + last two, "2 earlier replies"), surviving renderAll()', a.present && a.n === 3 && a.fold && a.fold.text.trim() === '2 earlier replies' && a.fold.expanded === 'false', JSON.stringify(a));
  ...
  ok('case D: fold opens inside the Resolved panel too', d.present && d.n === 5 && d.fold && d.fold.expanded === 'true', JSON.stringify(d));
  ...
  ok('case D: fold stays open across renderAll() while the panel is open', d.present && d.n === 5 && d.fold && d.fold.expanded === 'true', JSON.stringify(d));
  ...
  await sleep(100);
  const f1 = await J(`JSON.stringify({cls:document.activeElement.className, idx:document.activeElement.closest('.gentry')?.dataset.idx, inD:!!document.activeElement.closest(${JSON.stringify(D)})})`);
  ok('focus: after Show more in the Resolved panel, focus stays on that entry\'s toggle', f1.inD && f1.idx === '3' && /gmore/.test(f1.cls), JSON.stringify(f1));
